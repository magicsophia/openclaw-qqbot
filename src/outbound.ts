...
async function chunkedUploadAndSend(
  ...
  if (ctx.targetType === "c2c") {
    ...
    try {
      ...
      return { channel: "qqbot", error: `分片上传失败: ${msg}` };
    }
  }
...
async function sendFallbackLink(
  ...
  try {
    ...
    if (ctx.targetType === "c2c") {
      const r = await sendC2CMessage(token, ctx.targetId, fallbackText, ctx.replyToId);
      return {
        channel: "qqbot",
        messageId: r.id,
        timestamp: r.timestamp,
        error: `${caller}: 媒体发送失败 (${errorReason})，已发送链接供手动打开`,
      };
    } else if (ctx.targetType === "group") {
      const r = await sendGroupMessage(token, ctx.targetId, fallbackText, ctx.replyToId);
      return {
        channel: "qqbot",
        messageId: r.id,
        timestamp: r.timestamp,
      ...
      const r = await sendChannelMessage(token, ctx.targetId, fallbackText, ctx.replyToId);
      return {
        channel: "qqbot",
        messageId: r.id,
        timestamp: r.timestamp,
