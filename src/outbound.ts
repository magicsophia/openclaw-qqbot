      ...
      try {
        if (item.type === "text") {
          ...
          if (isHttpUrl) {
            ...
            if (target.type === "c2c") {
              const result = await sendC2CFileMessage(accessToken, target.id, undefined, filePath, replyToId ?? undefined);
              lastResult = { channel: "qqbot", messageId: result.id, timestamp: result.timestamp };
            } else if (target.type === "group") {
              const result = await sendGroupFileMessage(accessToken, target.id, undefined, filePath, replyToId ?? undefined);
              lastResult = { channel: "qqbot", messageId: result.id, timestamp: result.timestamp };
            } else {
            ...
            console.log(`[qqbot] sendText: Read local file (${fileBuffer.length} bytes): ${filePath}`);

            if (target.type === "c2c") {
              const result = await sendC2CFileMessage(accessToken, target.id, fileBase64, undefined, replyToId ?? undefined);
              lastResult = { channel: "qqbot", messageId: result.id, timestamp: result.timestamp };
            } else if (target.type === "group") {
              const result = await sendGroupFileMessage(accessToken, target.id, fileBase64, undefined, replyToId ?? undefined);
              lastResult = { channel: "qqbot", messageId: result.id, timestamp: result.timestamp };
            } else {
              const result = await sendChannelMessage(accessToken, target.id, `[文件消息暂不支持频道发送]`, replyToId ?? undefined);
              lastResult = { channel: "qqbot", messageId: result.id, timestamp: result.timestamp };
...
async function sendVoiceFile(ctx: MediaOutboundContext): Promise<OutboundResult> {
  ...
  try {
    ...
    if (!silkBase64) {
      ...
      const buf = fs.readFileSync(mediaUrl);
      const fallbackBase64 = buf.toString("base64");
      console.log(`[qqbot] sendVoiceFile: not SILK format, uploading raw file (${buf.length} bytes)`);
...
async function sendDocumentFile(ctx: MediaOutboundContext): Promise<OutboundResult> {
  ...
  try {
    ...
    if (isHttpUrl) {
      ...
      if (target.type === "c2c") {
        fileResult = await sendC2CFileMessage(accessToken, target.id, undefined, mediaUrl, replyToId ?? undefined);
      } else if (target.type === "group") {
        fileResult = await sendGroupFileMessage(accessToken, target.id, undefined, mediaUrl, replyToId ?? undefined);
      } else {
      ...
      const fileBase64 = fileBuffer.toString("base64");
      console.log(`[qqbot] sendDocumentFile: read local file (${fileBuffer.length} bytes), uploading...`);

      if (target.type === "c2c") {
        fileResult = await sendC2CFileMessage(accessToken, target.id, fileBase64, undefined, replyToId ?? undefined);
      } else if (target.type === "group") {
        fileResult = await sendGroupFileMessage(accessToken, target.id, fileBase64, undefined, replyToId ?? undefined);
      } else {
        const r = await sendChannelMessage(accessToken, target.id, `[文件消息暂不支持频道发送]`, replyToId ?? undefined);
        return { channel: "qqbot", messageId: r.id, timestamp: r.timestamp };
      }
